...
from sklearn_rvm.rvm import RVR
...
class TD3RVM:
    def __init__(self, buffer_size=1000000, batch_size=128, start_steps=10000, update_after=1000, update_every=50, policy_delay=2, rvm_delay=1000, action_noise_params=[0, 0.2], target_noise_params=[0, 0.2, 0.2], gamma=0.99, q_lr=1e-4, policy_lr=1e-4, polyak=0.995, device=DEFAULT_DEVICE):
        self.device = device
        ...
        # initialize rvm for guided noise generation
        self.rvm_delay = rvm_delay
        self.rvm = RVR(kernel="rbf", gamma="scale")
    ...
    def guided_policy_action(self, s):
        a = self.policy_action(s)
        
        x = torch.cat([s, a], dim=-1)

        prediction, uncertainty = self.rvm.predict(x.detach().cpu().numpy().reshape(1, -1), return_std=True)
        scale = np.abs(uncertainty[0]/prediction[0])

        noise = torch.normal(mean=self.target_noise_params[0], std=self.target_noise_params[1] * scale, size=a.shape).to(self.device)
        return torch.clamp(a + noise, min=-1, max=1)
    ...
    def fit_rvm(self):
        ...
        # fit the rvm
        with torch.no_grad():
            q = self.q1_target(s, a)

        x = torch.cat([s, a], dim=-1)
        
        self.rvm.fit(x.detach().cpu().numpy(), q.detach().cpu().numpy().squeeze())
    ...
    def train(self, num_episodes=5000):
        ...
        while not env.done():
            ...
            elif steps > self.rvm_delay:
                a = self.guided_policy_action(s)
            else:
